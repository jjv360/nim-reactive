...
dynamicImportFromData(dllName, dllData):
    ...
    proc GetAvailableCoreWebView2BrowserVersionString(browserExecutableFolder: PCWSTR = nil, versionInfo: ptr LPWSTR) {.stdcall.}

    ## Environment create callback
    type ICoreWebView2CreateCoreWebView2EnvironmentCompletedHandler = proc(self: pointer, errorCode: HRESULT, env: pointer) {.stdcall.}

    ## Creates an evergreen WebView2 Environment using the installed WebView2 Runtime version.
    proc CreateCoreWebView2Environment(environmentCreatedHandler: ICoreWebView2CreateCoreWebView2EnvironmentCompletedHandler) {.stdcall.}
...
class WebView2:
    ...
    method versionInfo(): string {.static.} =
        ...
        var wStr : LPWSTR
        GetAvailableCoreWebView2BrowserVersionString(nil, wStr.addr)
        let str = $wStr
        CoTaskMemFree(wStr)
        return str
